...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_text, gpt4o_latency = call_openai(
        model=OPENAI_MODEL,
        prompt=prompt
    )
    mini_text, mini_latency = call_openai_mini(
        prompt=prompt
    )
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history.append({"role": "system", "content": reply_text})
        history = history[-6:]  # giữ 3 lượt cuối (6 message)
...
def run_assistant(
    ...
    while True:
        # Kiểm tra trước khi đọc input để max_turns=0 thoát ngay lập tức.
        if max_turns is not None and num_turns >= max_turns:
            break
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def shorten(value: Any) -> str:
        """Đưa nội dung về một dòng và cắt gọn nếu dài quá 40 ký tự."""
        text = " ".join(str(value).splitlines())
        if len(text) > text_width:
            return text[: text_width - 3] + "..."
        return text
    ...
    header = (
        f"{headers[0]:<{text_width}} | "
        f"{headers[1]:<{text_width}} | "
        f"{headers[2]:<{text_width}} | "
        f"{headers[3]:>{latency_width}} | "
        f"{headers[4]:>{latency_width}}"
    )
    separator = "-" * len(header)
    rows = [header, separator]

    for result in results:
        gpt4o_latency = f'{result["gpt4o_latency"]:.4f}s'
        mini_latency = f'{result["mini_latency"]:.4f}s'
        rows.append(
            f'{shorten(result["prompt"]):<{text_width}} | '
            f'{shorten(result["gpt4o_response"]):<{text_width}} | '
            f'{shorten(result["mini_response"]):<{text_width}} | '
            f"{gpt4o_latency:>{latency_width}} | "
            f"{mini_latency:>{latency_width}}"
        )
